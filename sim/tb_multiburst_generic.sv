// tb_multiburst_generic — TEST-FIRST regression for the GENERIC-PHY MULTI-BURST read over-stream hang.
//
// GENERIC-variant parity for tb_multiburst (which proves the fix on the SDR PHY). hyperbus_phy_generic
// is the DEFAULT, vendor-free reference PHY and had the SAME over-stream leak: a single 16-word
// (= BURST_WORDS) read completes, but any LEN > BURST_WORDS (issued as several back-to-back 16-word
// Avalon reads) HANGS — the real W957D8NB OVER-STREAMS a few extra read words after the master stops
// CK (its read output pipeline drains past the CK-stop), so stray words linger in the read FIFO;
// hyperbus_ctrl's ST_IDLE gates cmd_ready on rd_fifo_empty and never sees an empty FIFO, so it never
// accepts the next burst. The GENERIC-PHY fix (disarm flush of both FIFO pointers on !phy_rd_arm +
// a deeper 32-entry RX FIFO) discards the over-stream tail at each burst boundary.
//
// This TB drives the EXACT board stack — bench engine (hyperram_bw_test) -> hyperram_avalon
// (PHY_VARIANT="GENERIC", RD_PREAMBLE_SKIP=1) -> golden model — but the model now emits the read
// over-stream (hyperram_model RD_OVERSTREAM_WORDS = OS_WORDS) AND the read preamble
// (RD_PREAMBLE_CLOCKS=1), exactly as the silicon does. It uses a REAL 90-degree clk/clk90 pair (as
// tb_avalon), NOT the SDR 2x byte clock, because the GENERIC PHY recovers read data source-
// synchronously off a ~90-degree-shifted RWDS strobe. It runs the bench for LEN = 16 (single burst,
...
module tb_multiburst_generic;
  ...
  localparam real                   F_CLK   = 100.0e6;             // word-clock (CK) rate for MB/s

  // --------------------------------------------------------------------
  // Clocking / reset — GENERIC arrangement (as tb_avalon): a REAL 90-degree pair.
  //   clk   = 100 MHz (CK-rate word clock; controller + bench)
  //   clk90 = 100 MHz, phase-shifted +90 deg (+2.5 ns) — centres CK on the DQ eye (NOT a 2x byte clock)
  // --------------------------------------------------------------------
  ...
  // Round-trip DQ/RWDS flight delay (device -> master), as in tb_avalon.
  ...
  initial begin
    ...
    $display("==================================================================");
    $display("tb_multiburst_generic: burst-length sweep (BURST_WORDS=%0d, over-stream=%0d words)",
    ...
    $display("==================================================================");
    $display("[%0t] tb_multiburst_generic done: %0d errors", $time, errors);
    if (errors == 0) begin
      $display("TB_RESULT: PASS");
      $finish;
    end else begin
      $display("TB_RESULT: FAIL");
      $fatal(1, "tb_multiburst_generic: %0d errors", errors);
    end
  end
  ...
  initial begin
    ...
    $fatal(1, "tb_multiburst_generic: global timeout");
  end
