// tb_sdr — self-checking Verilator testbench for hyperram_avalon with PHY_VARIANT="SDR".
//
// Same rigor as tb_avalon (POR init + CR0, single/burst memory write-then-read-back, a wrap-group-
// crossing burst, a CR0 register write+readback, an ID0 read) but exercises the portable single-clock-
// phase SDR PHY (rtl/phy/hyperbus_phy_sdr.sv) instead of the behavioural DDR generic PHY.
//
// CLOCKING (the SDR variant is 2:1 mesochronous — see hyperbus_phy_sdr.sv header):
//   * clk    = 50 MHz  — the CK-rate WORD clock. Clocks the Avalon front-end + controller. hb_ck runs
//              at this rate. All Avalon stimulus is synchronous to this clock (as in tb_avalon).
//   * clk90  = 100 MHz — REPURPOSED as the 2x BYTE clock (single "PLL", 0deg). Clocks the PHY's SDR
//              output registers, the hb_ck generator, and the read capture. Phase-aligned to clk
//              (both edges coincide at t=0,20,40,... exactly as an integer PLL divide would give).
//   * clk_ref= tie-off.
// This is the on-board arrangement (IOPLL: 100 MHz to the periphery, 50 MHz to the fabric/controller).
//
// Data is address-derived (genword) so every read-back word is checked independently. Any mismatch
// -> error count > 0 -> $fatal (non-zero exit). Success -> "TB_RESULT: PASS" + $finish.
//
...
module tb_sdr;
  ...
  // Latency 6, fixed — matches tb_avalon so the model's post-CR0 latency lines up with the controller.
  ...
  // Round-trip DQ/RWDS flight delay (device -> master). Delaying the read path stresses read capture.
  ...
  // --------------------------------------------------------------------
  // DUT: Avalon top with the SDR PHY
  // --------------------------------------------------------------------
  hyperram_avalon #(
    .LATENCY_CLOCKS   (6),
    .FIXED_LATENCY    (1'b1),
  ...
  // --------------------------------------------------------------------
  // Avalon transaction tasks (identical structure to tb_avalon).
  // --------------------------------------------------------------------
  ...
  initial begin
    ...
    $display("==================================================================");
    $display("[%0t] tb_sdr done: %0d checks, %0d errors", $time, checks, errors);
    if (errors == 0) begin
      $display("TB_RESULT: PASS");
      $finish;
    end else begin
      $display("TB_RESULT: FAIL");
      $fatal(1, "tb_sdr: %0d errors", errors);
    end
  end

  // Global watchdog (SDR runs the word clock at 50 MHz => ~2x tb_avalon sim time).
  initial begin
    #4_000_000;
    $display("[%0t] TB_RESULT: FAIL (global timeout)", $time);
    $fatal(1, "tb_sdr: global timeout");
  end
