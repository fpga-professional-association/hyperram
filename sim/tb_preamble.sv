// tb_preamble — TEST-FIRST regression for the HyperRAM read-strobe PREAMBLE bug.
//
// Reproduces the AXC3000 on-silicon bring-up hang (fpga/axc3000/sysconsole/cap_sample_dump.txt) and
// proves the fix. The real Winbond W957D8NB drives a short RWDS PREAMBLE — RWDS toggles like CK with
// DQ Hi-Z (=0x00) for one CK cycle BEFORE the first real read byte (captured at cap idx85-88, real
// data from idx89). The SDR PHY armed its RWDS byte-pairer during the latency window, so those
// preamble edges paired into a PHANTOM {0x00,0x00} word that shifted the whole read: STATUS never
// reached `done`, av_waitrequest stuck High — the hang seen on hardware.
//
// This TB drives the SDR datapath (hyperram_avalon, PHY_VARIANT="SDR") against a device model that
// now emits that preamble (hyperram_model RD_PREAMBLE_CLOCKS=1). It instantiates the SAME datapath
// TWICE:
//   * DUT_BUG : SDR PHY RD_PREAMBLE_SKIP=0  -> must MIS-READ (the reproduction; wrong data / short read)
//   * DUT_FIX : SDR PHY RD_PREAMBLE_SKIP=1  -> must read back exactly what was written, and complete
// The TB PASSES iff the bug is reproduced on DUT_BUG *and* DUT_FIX is clean — i.e. the preamble skip
// both (a) is necessary and (b) is sufficient. Reads are bounded (guarded), so a reproduction that
// hangs on hardware surfaces here as a short/incomplete read rather than an infinite loop.
//
...
module tb_preamble;
  ...
  // --------------------------------------------------------------------
  // Clocking / reset: clk = 50 MHz word clock, clk90 = 100 MHz byte clock (2x, phase-aligned).
  // --------------------------------------------------------------------
  logic clk, clk90, clk_ref, rst;
  initial begin clk     = 1'b0; forever #10.0 clk     = ~clk;     end   // 50 MHz  (CK word clock)
  initial begin clk90   = 1'b0; forever #5.0  clk90   = ~clk90;   end   // 100 MHz (2x byte clock)
  ...
  // ==================================================================================================
  //  Helper to build one SDR datapath + golden model with the read preamble.  (Written out twice —
  ...
  localparam realtime RTT = 3.0;    // ns round-trip flight delay (as tb_sdr)
  ...
  hyperram_avalon #(
    ...
    .POR_DELAY_CYCLES (0), .INIT_CR0 (TB_INIT_CR0), .PHY_VARIANT ("SDR"), .DIFF_CK (1'b1),
    .RD_PREAMBLE_SKIP (0)                            // <-- unfixed PHY
  ) dut_bug (
    .clk (clk), .clk90 (clk90), .clk_ref (clk_ref), .rst (rst),
    // cal tied to constants reproducing RD_PREAMBLE_SKIP=0 (the unfixed/buggy PHY)
    ...
    .dbg_postwin_hold (1'b0), .wrap_en (1'b0)
  );
  hyperram_model #(
  ...
  hyperram_avalon #(
    ...
    .clk (clk), .clk90 (clk90), .clk_ref (clk_ref), .rst (rst),
    // cal tied to constants reproducing RD_PREAMBLE_SKIP=FIX_SKIP=1 (the fixed PHY)
  ...
  initial begin
    ...
    $display("==================================================================");
    $display("[%0t] tb_preamble: bug_reproduced=%0b fix_clean=%0b errors=%0d",
    ...
    if (errors == 0 && bug_reproduced && fix_clean) begin
      ...
      $fatal(1, "tb_preamble: %0d errors (bug_reproduced=%0b fix_clean=%0b)",
  ...
  initial begin
    ...
    $fatal(1, "tb_preamble: global timeout");
  end
